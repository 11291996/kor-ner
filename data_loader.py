...
data_dir = "./data"
label_file = "ner_label.txt"
test_file = "ner_test.tsv"
...
class NaverNerProcessor(object):
    ...
    def get_examples(self, mode):
        ...
        file_to_read = None
        if mode == 'train':
            file_to_read = self.path["train_file"]
        elif mode == 'dev':
            file_to_read = self.path["dev_file"]
        elif mode == 'eval':
            file_to_read = self.path["eval_file"]

        return self._create_examples(self._read_file(os.path.join(data_dir, file_to_read)), mode)
...
def load_and_cache_examples(path, max_seq_len, tokenizer, mode):

    processor = NaverNerProcessor(path)
    ...
    features = convert_examples_to_features(examples, max_seq_len, tokenizer, pad_token_label_id=pad_token_label_id)
